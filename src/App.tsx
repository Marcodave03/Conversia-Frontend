...
const App: React.FC<InterviewProps> = () => {
  ...
  const [isInitialized, setIsInitialized] = useState(false); // New flag to track full initialization
  ...
  // Session restoration and wallet resolution
  useEffect(() => {
    const initializeSession = () => {
      console.log("🚀 [initializeSession] Starting session restoration...");
      
      // Try to get wallet address from various sources
      const zkWallet = localStorage.getItem("zkloginWallet");
      const bypassWallet = localStorage.getItem("bypassWallet");
      const suiWallet = wallet.account?.address;
      
      const resolvedWallet = suiWallet || bypassWallet || zkWallet;
      ...
      // Set wallet address if valid
      if (resolvedWallet?.startsWith("0x")) {
        console.log("✅ Valid wallet found:", resolvedWallet);
        setWalletAddress(resolvedWallet);
        
        // Try to restore userId from localStorage
        const storedUserId = localStorage.getItem("userId");
        if (storedUserId) {
          console.log("✅ Restoring userId from session:", storedUserId);
          setUserId(parseInt(storedUserId));
        }
      } else {
        console.log("⚠️ No valid wallet address found");
      }
      
      // Mark wallet as resolved
    ...
    const handleZkLoginSuccess = () => {
      console.log("🎉 ZkLogin success event received, reinitializing...");
      initializeSession();
    };
  ...
  useEffect(() => {
    const ensureUserExists = async () => {
      console.log("🧠 [ensureUserExists] Checking conditions:", {
        walletResolved,
        walletAddress,
        userId,
        isInitialized
      ...
      // Skip if wallet not resolved or no address
      if (!walletResolved || !walletAddress) {
        console.log("⏭️ Wallet not ready, skipping user creation");
        return;
      }

      // If we already have a userId, mark as initialized and skip API call
      if (userId) {
        console.log("✅ UserId already exists, marking as initialized");
    ...
    ensureUserExists();
  }, [walletResolved, walletAddress, userId, isInitialized]);
